...
places_data = {
    ...
    "Lacchwala": Place("Lacchwala", (30.2536, 78.1087), 40, 4.2, "Picnic spot with water activities", 2.5, "Adventure", 0.65),
...
# Simplified graph without elevation data
graph = {
    ...
    "Maldevta": {"Robber's Cave": {"dist": 6, "time": 18}, "Sahastradhara": {"dist": 5, "time": 15}, "Lacchwala": {"dist": 7, "time": 20}},
    "Buddha Temple": {"Mindrolling Monastery": {"dist": 2, "time": 8}, "Tapkeshwar Temple": {"dist": 3, "time": 10}, "Paltan Bazaar": {"dist": 4, "time": 12}},
    "Lacchwala": {"Maldevta": {"dist": 7, "time": 20}, "Rajaji National Park": {"dist": 8, "time": 25}},
    "Rajaji National Park": {"Lacchwala": {"dist": 8, "time": 25}}
}
...
class SimplifiedTravelGuideApp:
    ...
    def _create_extended_forecast_display(self, parent, location: str, forecasts: list):
        ...
        dates = [f.split('-')[2] for f, _ in forecasts]  # Day numbers
        temps = [w['temp_c'] for _, w in forecasts]
        
        ax.plot(dates, temps, marker='o', linewidth=3, markersize=8, color='#1f77b4')
        ax.set_xlabel('Day', color='white')
        ax.set_ylabel('Temperature (°C)', color='white')
        ax.set_title('Temperature Trend', color='white', fontsize=14, fontweight='bold')
...
💰 Cost: ₹{route_info.cost}"""
    ...
    def _create_status_bar(self):
        ...
        status_bar = ctk.CTkFrame(self.main_container, height=30, corner_radius=0)
        status_bar.pack(fill="x", side="bottom")
        
        status_text = ctk.CTkLabel(
            status_bar,
            text="Ready | Smart Travel Guide v2.0 | DAA Algorithms: A*, BST, Heap, Caching",
            font=ctk.CTkFont(size=10)
        )
